...
class Seed(Dataset):
    ...
    def get_next_pos(self, random_generator, indices, which_seed):
        ...
        half_voxel_range = [self.size[0] / 2, self.size[1] / 2, self.size[2] / 2]
        ...
        r_x = random_generator.uniform(-half_voxel_range[0], half_voxel_range[0])
        r_y = random_generator.uniform(-half_voxel_range[1], half_voxel_range[1])
        r_z = random_generator.uniform(-half_voxel_range[2], half_voxel_range[2])

        return x * self.size[0] + r_x, y * self.size[1] + r_y, z * self.size[2] + r_z
