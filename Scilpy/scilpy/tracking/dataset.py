...
class Dataset(object):
    ...
    def __init__(self, img, interpolation='trilinear'):
        ...
        self.size = img.get_header()['pixdim'][1:4]
        self.data = img.get_data(caching='unchanged').astype(np.float64)
        self.flipX = ((img.get_header()['sform_code'] > 0 and
                       img.get_header().get_sform()[0][0] < 0) or
                      (img.get_header()['qform_code'] > 0 and
                       img.get_header().get_qform()[0][0] < 0))
        if self.flipX:
