...
ANTS_MAX_ITERATIONS="50x50x25x10"
ANTS_TRANSFORMATION="SyN[0.2,3,0]"
ANTS_LINEAR_METRIC_PARAMS="1,32,Regular,0.25"
ANTS_LINEAR_CONVERGENCE="[500x250x125x50,1e-6,10]"
ANTS_METRIC="CC"
...
N4_CONVERGENCE_1="[50x50x50x50,0.000001]"
N4_CONVERGENCE_2="[50x50x50x50,0.000001]"
      ...
      do
        ...
        if [[ ! -f ${N4_CORRECTED_IMAGE} ]];
          then
            ...
            exe_n4_correction="${N4} -d ${DIMENSION} -i ${N4_TRUNCATED_IMAGE} -s ${N4_SHRINK_FACTOR_1} -c ${N4_CONVERGENCE_1} -b ${N4_BSPLINE_PARAMS} -o ${N4_CORRECTED_IMAGE} ${verbose}"
...
#                 exe_initial_align="${exe_initial_align} ${EXTRACTION_REGISTRATION_MASK}"
          ...
          basecall="${ANTS} -d ${DIMENSION} -u 1 -w [0.025,0.975] -o ${EXTRACTION_WARP_OUTPUT_PREFIX} -r ${EXTRACTION_INITIAL_AFFINE} -z 1 --float ${USE_FLOAT_PRECISION} ${verbose}"
          if [[ -f ${EXTRACTION_REGISTRATION_MASK} ]];
            then
              basecall="${basecall} -x [${EXTRACTION_REGISTRATION_MASK}]"
            fi
          stage1="-m MI[${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},${ANTS_LINEAR_METRIC_PARAMS}] -c ${ANTS_LINEAR_CONVERGENCE} -t Rigid[0.2] -f 8x4x2x1 -s 4x2x1x0"
          stage2="-m MI[${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},${ANTS_LINEAR_METRIC_PARAMS}] -c ${ANTS_LINEAR_CONVERGENCE} -t Affine[0.2] -f 8x4x2x1 -s 4x2x1x0"
          stage3="-m MI[${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},1,32,Regular,0.25] -m MI[${EXTRACTION_TEMPLATE_LAPLACIAN},${EXTRACTION_LAPLACIAN},1,32,Regular,0.25] -c [200x100x50,1e-6] -t ${ANTS_TRANSFORMATION} -f 4x2x1 -s 2x1x0"
      ...
      if [[ ! -f ${EXTRACTION_SEGMENTATION} ]];
        ...
        exe_brain_extraction_2="${WARP} -d ${DIMENSION} -i ${EXTRACTION_PRIOR} -o ${EXTRACTION_MASK_PRIOR_WARPED} -r ${ANATOMICAL_IMAGES[0]} -n Gaussian -t [${EXTRACTION_GENERIC_AFFINE},1] -t ${EXTRACTION_INVERSE_WARP} --float ${USE_FLOAT_PRECISION} ${verbose}"
