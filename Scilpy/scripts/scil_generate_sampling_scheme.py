...
def buildArgsParser():
    ...
    p.add_argument(
        ...
        metavar='b0_every', type=int, default=10,
        help='Interleave a b0 every b0_every. \nNo b0 if 0. \nOnly 1 b0 at beginning if > number of samples. [%(default)s]')
    p.add_argument(
        '--no-b0end', action='store_false', dest='b0_end', default=True,
        help='Don\'t add a b0 as last sample. [%(default)s]')
    p.add_argument(
        '--b0value', dest='b0_value', action='store',
        metavar='b0_value', type=float, default=0.0,
    ...
    p.add_argument(
        '--caru', action='store_true', dest='caru', default=False,
        help='Save in caruyer format (.caru). [%(default)s]')
    p.add_argument(
        '--phil', action='store_true', dest='phil', default=False,
        help='Save in Philips format (.txt). [%(default)s]')
    p.add_argument(
        '--fsl', action='store_true', dest='fsl', default=False,
        help='Save in FSL format (.bvecs/.bvals). [%(default)s]')
    p.add_argument(
        '--siemens', action='store_true', dest='siemens', default=False,
        help='Save in Siemens format (.dir). [%(default)s]')
...
def main():
    ...
    if not (caru or phil or fsl or siemens):
    ...
    if args.bvalues is not None:
        ...
        bvalues = bvalue_lin_b(bmin = 0.0, bmax = args.b_lin_max, nb_of_b_inside = S - 1, exclude_bmin = True)
    elif args.q_lin_max is not None:
        bvalues = bvalue_lin_q(bmin = 0.0, bmax = args.q_lin_max, nb_of_b_inside = S - 1, exclude_bmin = True)
    # Add b0 b-value
    if b0_every != 0:
    ...
    # Scheme generation
    points, shell_idx = gen_scheme(Ks, verbose =  int(3 - logging.getLogger().getEffectiveLevel()//10))
    ...
    if b0_every != 0:
        points, shell_idx = add_b0s(points, shell_idx, b0_every = b0_every, finish_b0 = b0_end)

    # duty cycle optimization
    if duty:
        points, shell_idx = min_duty_cycle_bruteforce(points, shell_idx, bvalues)
