...
files flags" section.\n\n\nIf --not_all is set, only the metrics specified
...
dyy, dyz, dzz).\n\n\nFor all the quality control metrics such as residual,
physically implausible signals, pulsation and misalignment artifacts, see the
paper [J-D Tournier, S. Mori, A. Leemans. Diffusion Tensor Imaging and Beyond.
MRM 2011].
"""
...
def _build_args_parser():
    ...
    p.add_argument(
        ...
        help='If set, will only save the metrics explicitely specified using '
...
def main():
    ...
    if bvals.min() != 0:
        if bvals.min() > 20:
            raise ValueError(
                'The minimal b-value is greater than 20.\nThis is highly '
