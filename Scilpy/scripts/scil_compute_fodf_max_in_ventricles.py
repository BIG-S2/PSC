...
def load(path):
    img = nib.load(path)
    return img.get_data(), img.get_affine(), img.get_header().get_zooms()[:3]
...
def get_ventricles_max_fodf(data, fa, md, zoom, args):
    ...
    b_matrix = get_b_matrix(order, sphere, args.basis)
    sum_of_max = 0
    count = 0
    ...
    all_i = range(int(data.shape[0]/2) - step, int(data.shape[0]/2) + step)
    all_j = range(int(data.shape[1]/2) - step, int(data.shape[1]/2) + step)
    all_k = range(int(data.shape[2]/2) - step, int(data.shape[2]/2) + step)
    for i in all_i:
        for j in all_j:
            for k in all_k:
