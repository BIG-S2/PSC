...
for i=1:L
	...
	% temp2=temp2(3:end);
	% l03=length(temp2);
	% if (l00>14)&(l01==8)&(l02==3)&(l03==2) 
	% continue;
	% end
	
    fid = fopen(sprintf('%s/jobsubmission_stg1_%s.pbs',codedir,subID{i}),'w');
    fprintf(fid,'#PBS -l nodes=1:rhel7:ppn=10 -l walltime=23:59:59,mem=80gb\n');
	...
	fprintf(fid,'setenv SCIP_PATH %s\n',scriptdir);
	fprintf(fid,'cd %s/%s \n',datadir,sub_id);
    %remove output1 and prepare for re-write
    fprintf(fid,'chmod 775 %s/Connectome_Step1.sh \n',scriptdir1);
	fprintf(fid,'sh %s/Connectome_Step1.sh \n',scriptdir1);
